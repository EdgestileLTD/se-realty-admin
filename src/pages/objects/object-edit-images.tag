    ...
    .row
        .col-md-12
            ...
            catalog='{ catalog }', upload='{ upload }', remove='true', reorder='true', sortable='{ sort }')
                ...
                #{'yield'}(to='body')
                    datatable-cell(name='')
                        img(src!='{ handlers.rootImagePath + row.imagePath }', height='64px', width='64px')
                    datatable-cell(name='')
                        p.form-control-static { row.imagePath }
                        input.form-control(value='{ row.alt }', onchange='{ handlers.imageAltChange }')
                    datatable-cell(name='', style='width: 20px;')
                        i.fa.fa-check-square-o.fa-2(if='{ row.isMain }')
                        i.fa.fa-square-o.fa-2(if='{ !row.isMain }', onclick='{ handlers.isMainChange }')
