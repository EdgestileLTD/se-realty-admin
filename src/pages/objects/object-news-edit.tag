...
object-news-edit
    ...
    virtual(hide='{ loader }')
        ...
        .btn-group
            a.btn.btn-default(href='#objects/{ item.idObject }') #[i.fa.fa-chevron-left]
    ...
    script(type='text/babel').
        ...
        observable.on('object-news-new', item => {
            let { group, name } = item
            self.error = false
            self.isNew = true
            self.item = {
        ...
        observable.on('object-news-edit', id => {
            var params = {id: id}
            self.error = false
            self.isNew = false
            self.loader = true
        ...
        self.reload = () => {
            self.item.id ? observable.trigger('object-news-edit', self.item.id) : observable.trigger('object-news-new')
        }
