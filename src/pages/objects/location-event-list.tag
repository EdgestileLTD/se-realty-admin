...
location-event-list
    catalog(object='LocationEvents', search='true', sortable='true', cols='{ cols }', reload='true',
    ...
    script(type='text/babel').
        var self = this
        self.id = 0
        self.objectFilters = [{field: 'idLocation', sign: 'IN', value: self.id }]

        self.mixin('permissions')
        self.mixin('remove')
        self.collection = 'LocationEvents'
        ...
        observable.on('location-event-reload', (id) => {
            self.id = id
            self.objectFilters = [{field: 'idLocation', sign: 'IN', value: self.id }]
