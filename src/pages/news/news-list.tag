...
news-list
            ...
            .well.well-sm
                .form-inline
                    .form-group
                        ...
                        select.form-control(data-name='idGroup', data-sign='IN', , onchange='{ parent.selectCategory }')
                            option(value='') Все
                            option(each='{ category, i in parent.newsCategories }', value='{ category.id }', no-reorder) { category.name }
        #{'yield'}(to='body')
    ...
    script(type='text/babel').
        ...
        self.selectCategory = e => {
            self.selectedCategory = e.target.value || 0
            self.selectedCategoryName = undefined
            if (self.selectedCategory > 0) {
                var items = self.newsCategories
                for(var i=0; i < items.length; i++){
                    if (items[i].id == self.selectedCategory)
                    self.selectedCategoryName = items[i].name
                }
            } else {
