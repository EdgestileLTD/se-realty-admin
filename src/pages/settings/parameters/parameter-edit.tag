...
| import 'pages/settings/parameters/feature-new-edit-modal.tag'
| import 'pages/settings/parameters/parameters-groups-list-modal.tag'

parameter-edit
    loader(if='{ loader }')
    div
        .btn-group
            a.btn.btn-default(href='#settings/parameter') #[i.fa.fa-chevron-left]
        ...
        .tab-content
            form(action='', onchange='{ change }', onkeyup='{ change }', method='POST')
                ...
                .row(if='{ item.type == "calc" }')
                    .col-md-12
                        label.control-label Формула ( f1 * f2 без скобок)
                        input.form-control(name='function', value='{ item.function }')
                .row
                    .col-md-3
                        .form-group
    ...
    script(type='text/babel').
        ...
        self.reload = () => {
            observable.trigger('parameter-edit', self.item.id)
        }
        ...
        observable.on('parameter-edit', id => {
            var params = {id: id}
            self.item = {}
            self.loader = true
            self.error = false
