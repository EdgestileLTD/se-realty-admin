...
| import 'pages/settings/parameters/parameter-new-modal.tag'
...
parameters-list
    ...
    dblclick='{ permission(parameterOpen, "products", "1000") }')
        #{'yield'}(to='body')
            datatable-cell(name='id') f{ row.id }
            datatable-cell(name='imageUrlPreview', style='max-width: 50px;')
    ...
    script(type='text/babel').
                    ...
                    if (!_this.error) {
                        API.request({
                            ...
                            success(response) {
                                _this.modalHide()
                                if (response.id)
                                    riot.route(`settings/parameter/${response.id}`)
                            }
                        })
                    }
                }
        ...
        self.parameterOpen = e => riot.route(`/settings/parameter/${e.item.row.id}`)

        self.cols = [
            {name: 'id', value: '#'},
            {name: 'imageFile', value: 'Иконка'},
